...
<body class="bg-[#F3F4F6] min-h-screen flex items-center justify-center p-4 font-['Inter']">
    ...
    <div class="flex flex-col md:flex-row w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden min-h-[550px]">
        ...
        <div class="w-full md:w-[55%] p-10 md:p-14 flex flex-col justify-center bg-white">
            ...
            <p class="text-[0.95rem] text-slate-500 mb-8 tracking-tight">Silakan daftar untuk mengakses panel bantuan Anda.</p>

            @if(session('success'))
                <div class="bg-green-50 text-green-600 text-sm p-4 rounded-xl border border-green-100 mb-6 font-medium">
                    {{ session('success') }}
                </div>
            @endif
            ...
            <form action="{{ route('register.store') }}" method="POST" class="space-y-5">
                ...
                <!-- Nama Lengkap -->
                <div>
                    <label for="name" class="block text-[0.7rem] font-bold text-slate-600 uppercase tracking-widest mb-2">
                        Nama Lengkap
                    </label>
                ...
                <!-- Password -->
                <div>
                    <label for="password" class="block text-[0.7rem] font-bold text-slate-600 uppercase tracking-widest mb-2">
                        Kata Sandi
                    </label>
                    <div class="relative flex items-center">
                        <div class="absolute pl-4 pointer-events-none text-slate-400">
                        ...
                        <input type="password" id="password" name="password" required
                            ...
                            placeholder="••••••••">
                        <button type="button" onclick="togglePassword('password', 'eye-icon-1')" class="absolute right-0 pr-4 text-slate-400 hover:text-slate-600 focus:outline-none transition-colors">
                            <svg id="eye-icon-1" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Konfirmasi Password -->
                <div>
                    <label for="password_confirmation" class="block text-[0.7rem] font-bold text-slate-600 uppercase tracking-widest mb-2">
                    ...
                    <div class="relative flex items-center">
                        ...
                        <input type="password" id="password_confirmation" name="password_confirmation" required
                            class="block w-full pl-11 pr-11 py-3.5 border-none bg-[#F0F2F5] rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:ring-2 focus:ring-[#1C2C4E] focus:bg-white transition-all outline-none tracking-widest"
                            placeholder="••••••••">
                        <button type="button" onclick="togglePassword('password_confirmation', 'eye-icon-2')" class="absolute right-0 pr-4 text-slate-400 hover:text-slate-600 focus:outline-none transition-colors">
                            <svg id="eye-icon-2" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
                        </button>
            ...
            <div class="mt-8 text-center text-sm">
                ...
                <a href="{{ route('login') }}" class="font-bold text-[#1F54CE] hover:text-[#11389A] transition-colors ml-1">
                    Masuk
                </a>
            </div>

            <!-- Footer Links -->
            <div class="mt-6 flex justify-center space-x-6 text-[0.7rem] text-slate-400 font-semibold">
